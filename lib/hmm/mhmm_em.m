function [LL, prior, transmat, mu, Sigma, mixmat] = ...
     mhmm_em(data, prior, transmat, mu, Sigma, mixmat, p_prior, p_trans, varargin);
...
while (num_iter <= max_iter) & ~converged
  ...
  if adj_mu | adj_Sigma
      [loglik, exp_num_trans, exp_num_visits1, postmix, m, ip, op] = ...
          ess_mhmm(prior, transmat, mixmat, mu, Sigma, data, cov_type);
  else
...
if strcmp(cov_type, 'diag')
    ...
    model.MinvC   = model.mu .* model.iC;               % d x k     mean weighted with covariance
    MuDst   = .5 * sum(model.mu .* model.MinvC,1);% 1 x k     norm of means (with covariance)
...
for ex=1:numex
  ...
  if mix
    ...
    if strcmp(cov_type, 'diag')
        [P, logl] = mixgauss_prob2(obs, model);
        [alpha, beta, gamma,  current_loglik1, xi_summed] = fwdback(prior, transmat, P);
        ...
        [B, ~, logOff] = mixgauss_prob(obs, mu, Sigma);
        [alpha, beta, gamma,  current_loglik, xi_summed] = fwdback(prior, transmat, B);
        current_loglik     = current_loglik + size(obs,2) * logOff;
    end
  end    
