function [B, B2, logOff] = mixgauss_prob(data, mu, Sigma, mixmat, unit_norm)
...
elseif ndims(Sigma)==3 % tied across M
  ...
  for j=1:Q
    ...
    if isposdef(Sigma(:,:,j))
      ...
      logProb(j,:)  = logB2;
    elseif sum(diag(Sigma(:,:,j))) == 0
      sprintf('mixgauss_prob: Sigma q=%d is all zero!\n', j)
    else
        ddd=Sigma(:,:,j)
        save('test.mat','ddd');
        pause()
  ...
  % scale all the values for numeric sability. (log probs would be maybe
  % better, though would require big changes in the forward-backword
  % algorithm (logOff can restor org prob) ll + T*logOff
  %TODO
  %logOff=0;
  logOff        = max(max(logProb)); %scale that the highest value have a prob of 1!
  B2(:,1,:)     = exp(logProb - logOff);
  %B2(:,1,:)     = gmm_softmax(logProb,1);
  %TODO sclae by column!!!
else % general case
  B2 = zeros(Q,M,T);
  for j=1:Q
    for k=1:M
