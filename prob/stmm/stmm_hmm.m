function hmm = stmm_hmm(Xdesc,Xloc,hopts)
   ...
   rs       = RandStream.create('mt19937ar','seed',hopts.seed);
    ...
    elseif strcmp(hopts.init,'rnd'),
        ...
        hmm.nu     = hopts.opts.nu * ones(1,NrK);
        hmm.trans   = normalize(rand(rs,NrK) + eye(NrK),2);
        hmm.k      = NrK;
    end
    ...
    fprintf('\t--> Train HMM\n'); 
    [LL, prior1, transmat1, mu1, Sigma1, ~] = shmm_em(seqFeat, hmm.prior, hmm.trans, hmm.mean, Sigma0, [], p_prior, p_trans, hmm.nu, 'max_iter',hopts.opts.MaxIter, 'cov_type', cov_type, 'adj_mu',adj_mu,'adj_Sigma', adj_sigma,  'adj_prior', adj_prior);
